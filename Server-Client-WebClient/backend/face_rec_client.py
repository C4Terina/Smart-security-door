    ...
    if results.multi_face_landmarks:
        ...
        for faceLms in results.multi_face_landmarks:
            ...
            if x_min >=0 and x_max >= 0 and y_min >= 0 and y_max >= 0:
                ...
                personid, conf = recognise.predict(gray_face)
                if conf >= 4 and conf <= 85:
                    if personid:
                        if flag:
                            ser.write(b'o')
                            flag = False
                            data_to_send = Data(entry_id = 0, camera_id = 1, person_id = randint(0,5), person_name = "Bob", time_recognised = datetime.now())
                            print(data_to_send)
                        ...
                        if elsapsed_time > 10:
                            flag = True
                        cv2.putText(frame, labels[int(personid)-1], (x_max, y_min), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 1, cv2.LINE_AA)
                    else:
